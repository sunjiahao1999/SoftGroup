...
from .decoder import SuperPointDecoder
from .criterion import SpfCriterion


class SuperpointFormer(nn.Module):
    def __init__(
        self,
        channels=32,
        num_blocks=7,
        ...
        self.decoder = SuperPointDecoder(**decoder_cfg)
        self.criterion = SpfCriterion(**criterion_cfg)
    ...
    def forward_train(
        ...
        **kwargs
        ...
        insts = [inst.to(feats.device) for inst in insts]
        # losses = {}
        ...
        output_feats = scatter_mean(output_feats, superpoints, dim=0)  # (B*M, media)
        batch_idxs = scatter_mean(batch_idxs, superpoints, dim=0).long()  # (B*M)

        B = batch_idxs.max() + 1
        batch_offsets = torch.searchsorted(batch_idxs, torch.arange(B, device='cuda'), right=True)
        batch_offsets = torch.cat((torch.tensor([0], device="cuda"), batch_offsets), 0)

        pred = self.decoder(output_feats, batch_offsets)
        losses = self.criterion(pred, insts)
    ...
    def point_wise_loss(self, semantic_scores, pt_offsets, semantic_labels, instance_labels, pt_offset_labels):
        losses = {}
    ...
    def instance_loss(
        ...
        ious_on_cluster = get_mask_iou_on_cluster(proposals_idx, proposals_offset, instance_labels, instance_pointnum)
        ...
        ious = get_mask_iou_on_pred(
            proposals_idx, proposals_offset, instance_labels, instance_pointnum, mask_scores_sigmoid_slice.detach()
        )
    ...
    def forward_test(
        ...
        **kwargs
        ...
        voxel_feats = voxelization(feats, p2v_map)
        input = spconv.SparseConvTensor(voxel_feats, voxel_coords.int(), spatial_shape, batch_size)
        output_feats = self.forward_backbone(
        ...
        if self.test_cfg.x4_split:
            ...
            superpoints = self.merge_4_parts(superpoints)
        ...
        pred = self.decoder(output_feats, batch_offsets)
        pred_instances = self.get_instances(scan_ids[0], pred, superpoints)
        ...
        #     )
        #     inst_feats, inst_map = self.clusters_voxelization(
        #         proposals_idx, proposals_offset, output_feats, coords_float, **self.instance_voxel_cfg
        #     )
        #     _, cls_scores, iou_scores, mask_scores = self.forward_instance(inst_feats, inst_map)
        #     pred_instances = self.get_instances(
        #         scan_ids[0], proposals_idx, semantic_scores, cls_scores, iou_scores, mask_scores
        #     )
        #     gt_instances = self.get_gt_instances(semantic_labels, instance_labels)
        #     ret.update(dict(pred_instances=pred_instances, gt_instances=gt_instances))
        ret = dict(scan_id=scan_ids[0],pred_instances=pred_instances, gt_instances=gt_instances)
        return ret
    ...
    def forward_grouping(self, semantic_scores, pt_offsets, batch_idxs, coords_float, grouping_cfg=None):
        ...
        for class_id in range(self.semantic_classes):
            ...
            idx, start_len = ballquery_batch_p(coords_ + pt_offsets_, batch_idxs_, batch_offsets_, radius, mean_active)
    ...
    def get_instances(self, scan_id, pred, superpoints):
        pred_labels = pred['labels']
        pred_masks = pred['masks']

        scores = F.softmax(pred_labels[0], dim=-1)[:, :-1]
        labels = (
            torch.arange(self.instance_classes, device=scores.device)
            .unsqueeze(0)
            .repeat(self.decoder_cfg.num_query, 1)
            .flatten(0, 1)
        )
        scores, topk_idx = scores.flatten(0, 1).topk(self.test_cfg.test_topk_per_scene, sorted=False)
    ...
    def clusters_voxelization(
        self, clusters_idx, clusters_offset, feats, coords, scale, spatial_shape, rand_quantize=False
    ):
